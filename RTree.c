...
int compare(const void *s1, const void *s2)
{
  ...
  Data e2 = (Data)s2;
  int cnt=0;
  while(cnt < DIMENSION-1 && e1->iData[cnt]==e2->iData[cnt]){
	cnt++;
  }
...
{
	RHdrNd rootHead= RinitHdrNd();
	RLstNd tempTree=NULL;
	// unsigned int n=dataHdrLst->uiCnt;
	unsigned int temp=0;
	for(int i=0;i<32;i++){
		if(n & (1<<i)){
			RHdrNd wrTree= RbuildWRTree(dataHdrLst,temp,temp+ (1<<i) -1);
			temp+= (1<<i);
			rootHead->uiCnt++;
			if(tempTree==NULL){
				tempTree=wrTree->ptrFirstNd;
				rootHead->ptrFirstNd=tempTree;
			}
			else
			{
				tempTree->ptrNextNd=wrTree->ptrFirstNd;
				tempTree=tempTree->ptrNextNd;
			}
		}
	}
	return rootHead;
...
{
	qsort(dataHdrLst->dataClstElem, dataHdrLst->uiCnt, sizeof(struct data), compare);
	// for(int i=0;i<10;i++){
	// 	printf("%lf   %lf   %lf\n",(dataHdrLst->dataClstElem+i)->iData[0],(dataHdrLst->dataClstElem+i)->iData[1],(dataHdrLst->dataClstElem+i)->iData[2]);
	// }
	RHdrNd rootHead= RinitHdrNd();
	RLstNd tempTree=NULL;
	unsigned int n=dataHdrLst->uiCnt;
...
RHdrNd buildLRTree(DataHdr dataHdrLst)
{
	...
	// }
	RHdrNd finalTree;
	for(unsigned int i=1;i<=dataHdrLst->uiCnt;i++){
		finalTree=buildLRTreeN(dataHdrLst,i);
		// printf("%u is done \n",i);
	}
	return finalTree;
}
